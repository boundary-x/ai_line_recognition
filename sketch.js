...
async function startCamera(){
 requestStop();const token=++cameraEpoch;cameraReady=false;cameraBusy=true;
 ...
 try{
  ...
  stream.getVideoTracks().forEach(t=>t.addEventListener('ended',()=>{if(token!==cameraEpoch)return;cameraReady=false;requestStop();$('camera-status').hidden=false;$('camera-status').textContent='카메라 연결이 종료되었습니다.';$('retry-camera').hidden=false;controls();}));
 }catch(error){
...
function frame(now){
 ...
 if(video.currentTime===lastVideoTime){if(sending&&now-lastFrame>750)requestStop();return;}
 ...
 try{
  ...
  if(sending&&now-lastSend>=100){lastSend=now;latest=result?'x'+result.x+' e'+result.error+' d1':'stop';flush();}
 }catch(error){engineReady=false;requestStop();releaseMats();engineStatus('인식 처리 오류: '+error.message+' 새로고침해주세요.','error');controls();}
}
...
async function flush(){
 ...
 try{
  while(target===rx&&(stopPending||(sending&&latest!==null))){
   const isStop=stopPending,value=isStop?'stop':latest,token=epoch;
