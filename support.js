...
(() => {
  ...
  support.innerHTML = `
    ...
    <div class="support-content">
      <p>라인의 위치와 오차를 확인하고 로봇 제어에 활용해보세요.</p>
      ...
      <details class="support-section" id="help-examples"><summary>마이크로비트 예제 코드</summary><div class="support-answer example-codes">
        ...
        <p class="support-caption">이름 확인 코드는 로봇 주행 코드가 아닙니다. 로봇 코드에서 수신 데이터와 stop을 처리하고, 일정 시간 데이터가 들어오지 않으면 모터가 정지하도록 설정하세요.</p>
      </div></details>
      <details class="support-section" id="help-troubleshooting"><summary>문제 해결 <span class="support-meta">증상별 안내</span></summary><div class="support-answer support-faq">
        ...
        <details><summary>로봇이 반대 방향으로 움직여요</summary><p>화면 중앙은 x200입니다. 화면 왼쪽의 라인은 e가 양수, 오른쪽은 음수입니다. 좌우 반전을 바꾸면 화면과 데이터 방향이 함께 바뀝니다. 로봇 코드의 모터 방향과 오차 부호를 확인하세요.</p></details>
        <details><summary>블루투스 연결이나 전송이 안 돼요</summary><p>마이크로비트에 블루투스 UART 수신 코드가 있어야 합니다. 다른 앱과의 연결을 해제하고 지원 브라우저에서 연결하세요. 연결 후 ‘전송 시작’을 눌러야 데이터가 전송됩니다. ‘전송됨’은 브라우저의 쓰기 성공을 뜻하며 로봇의 동작 완료를 뜻하지 않습니다.</p></details>
      ...
      <details class="support-section"><summary>업데이트 노트 <span class="support-meta">최근 변경</span></summary><div class="support-answer"><p class="support-release">2026.09.21</p><ul><li>인식 미리보기와 블루투스 전송 분리</li><li>카메라 비율·반응형 화면과 상태 안내 개선</li><li>정지 명령 우선 전송 및 연결 해제 처리</li><li>사용법 둘러보기와 증상별 문제 해결 추가</li></ul></div></details>
    </div>`;
  const allSteps = [
    ...
    ['#help-examples','기기 코드를 준비하세요','이름 확인 코드로 장치 이름을 확인하세요. 프로젝트 예제는 준비 중입니다. 블루투스 UART와 stop을 처리하는 코드가 있어야 로봇을 제어할 수 있습니다.'],
    ['#bluetooth-control-buttons','마이크로비트를 연결하세요','기기 연결을 누르고 내 장치를 선택하세요. 연결만으로 데이터가 전송되지는 않습니다.'],
    ['#object-control-buttons','확인한 결과를 전송하세요','전송 시작을 누르면 x·e·d 값을 전송합니다. 라인이 없으면 stop을 보냅니다. 전송 중지 후에도 인식 미리보기는 계속됩니다.'],
    ['#dataDisplay','실제 전송 상태를 확인하세요','전송됨 표시는 블루투스 쓰기 성공 후 표시됩니다. 연결과 전송 오류가 있으면 이 영역의 안내를 확인하세요.']
  ];
  const chapters = [{label:'카메라',start:0},{label:'인식 설정',start:2},{label:'기기 연결·전송',start:6}];
